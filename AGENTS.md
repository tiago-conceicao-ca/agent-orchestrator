...
Monorepo (pnpm) with packages: `core`, `cli`, `web`, and `plugins/*`. The web dashboard is a Next.js 15 app (App Router) with React 19 and Tailwind CSS v4. Data flows from `cahi.yaml` through core's `loadConfig()` to API routes, served via SSR and a 5s-interval SSE stream. Terminal sessions use WebSocket connections to tmux PTYs. See CLAUDE.md for the full plugin architecture (8 slots), session lifecycle, and data flow.
...
Full guidelines with CAHI-specific context: see "Working Principles" in CLAUDE.md.
...
- `cahi stop` loads global config to see all projects; `cahi stop <project>` only kills that project's sessions
- Ctrl+C on `cahi start` performs full graceful shutdown (same as `cahi stop`)
- `LastStopState` includes `otherProjects` for cross-project session restore on next `cahi start`
