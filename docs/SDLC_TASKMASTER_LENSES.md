...
`taskmaster-main`) onto AO's SDLC engine, records the invariants the
implementation preserves, and explains the two deliberate deviations.

## taskmaster → AO mapping
...
1. **Bounded auto re-dispatch instead of a human gate.** When a review pass
   returns `needs_fixes`, AO auto re-dispatches the SAME pass with the review
...
2. **Per-task shared worktree, not one shared worktree for everything.** A
   ...
   worktree (AO's collision-safe isolation). No single-worktree-for-all-tasks
   rewrite.
...
- **SDLC-only worktree scoping.** Worktree reuse is an explicit opt-in:
  ...
  `worktrees/{sessionId}` path is untouched — every normal (non-SDLC) `ao spawn`
