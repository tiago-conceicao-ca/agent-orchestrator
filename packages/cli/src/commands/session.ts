...
export function registerSession(program: Command): void {
    ...
    .action(
      async (
        ...
        if (!resolvedSession) {
          console.error(
            chalk.red(
              "No session provided. Pass a session name or run this inside a managed CAHI session.",
            ),
          );
          process.exit(1);
        }
  ...
  session
    .command("remap")
    .description("Re-discover and persist OpenCode session mapping for an CAHI session")
