...
function resolveProject(
  ...
  if (ids.length === 0) {
    console.error(chalk.red("No projects configured. Run `cahi start` first."));
    process.exit(1);
  }
  if (ids.length > 1) {
    console.error(
...
async function getTracker(
  ...
  const { createPluginRegistry } = await import("@contaazul/cahi-core");
    ...
    .action(
      async (
        ...
        try {
          ...
          console.error(chalk.red("No config found. Run `cahi start` first."));
          process.exit(1);
          return;
        }
