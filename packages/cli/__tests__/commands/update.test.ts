...
describe("update command", () => {
  ...
  describe("npm-global install", () => {
    ...
    it("prints command and exits cleanly in non-TTY mode without prompting", async () => {
      ...
      expect(allOutput).toContain("npm install -g @contaazul/cahi@latest");
    });
  ...
  describe("update lifecycle orchestration", () => {
    beforeEach(() => {
      ...
      mockGetUpdateCommand.mockImplementation((method: string) => {
        if (method === "pnpm-global") return "pnpm add -g @contaazul/cahi@latest";
        return "npm install -g @contaazul/cahi@latest";
      });
      mockCheckForUpdate.mockResolvedValue(
        makeNpmUpdateInfo({
          installMethod: "pnpm-global",
          recommendedCommand: "pnpm add -g @contaazul/cahi@latest",
        }),
      );
      Object.defineProperty(process.stdin, "isTTY", { value: true, configurable: true });
      Object.defineProperty(process.stdout, "isTTY", { value: true, configurable: true });
    ...
    it("runs stop → package install → version verification → start/restore when sessions are active", async () => {
      ...
      expect(mockSpawn.mock.calls[0]).toEqual([
        ...
        expect.objectContaining({
          stdio: "inherit",
          env: expect.objectContaining({ CAHI_CONFIG_PATH: "/tmp/test-global-config.yaml" }),
        }),
      ]);
    ...
    it("forces an explicit prompt when active channel differs from cached.channel and !isOutdated", async () => {
      ...
      mockCheckForUpdate.mockResolvedValue(
        makeNpmUpdateInfo({
          ...
          isOutdated: false,
          recommendedCommand: "npm install -g @contaazul/cahi@nightly",
        }),
      );
      mockPromptConfirm.mockResolvedValue(true);
  ...
  describe("API-invoked install (CAHI_NON_INTERACTIVE_INSTALL=1)", () => {
    ...
    beforeEach(() => {
      ...
      Object.defineProperty(process.stdin, "isTTY", { value: false, configurable: true });
      Object.defineProperty(process.stdout, "isTTY", { value: false, configurable: true });
      origNonInteractive = process.env["CAHI_NON_INTERACTIVE_INSTALL"];
      process.env["CAHI_NON_INTERACTIVE_INSTALL"] = "1";
      mockSpawn.mockReturnValue(createMockChild(0));
    });

    afterEach(() => {
      if (origNonInteractive === undefined) {
        delete process.env["CAHI_NON_INTERACTIVE_INSTALL"];
      } else {
        process.env["CAHI_NON_INTERACTIVE_INSTALL"] = origNonInteractive;
      }
    });

    it("invokes runNpmInstall exactly once when CAHI_NON_INTERACTIVE_INSTALL=1 even though isTTY is false", async () => {
    ...
    it("preserves the old 'print Run:' behavior for non-API non-TTY (piped output)", async () => {
      ...
      await program.parseAsync(["node", "test", "update"]);
      const all = logSpy.mock.calls.map((c) => String(c[0])).join("\n");
      expect(all).toMatch(/Run: npm install -g @contaazul\/cahi@latest/);
      expect(mockSpawn).not.toHaveBeenCalled();
    });
  ...
  describe("runNpmInstall — cross-platform spawn options", () => {
    ...
    it.each([
      ["pnpm-global" as const, "pnpm add -g @contaazul/cahi@latest"],
      ["bun-global" as const, "bun add -g @contaazul/cahi@latest"],
