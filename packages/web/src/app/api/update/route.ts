/**
 * POST /api/update — kick off `cahi update` from the dashboard banner.
 *
 ...
 * user re-runs `cahi start` to pick up the new version.
 */
...
export async function POST(_req: NextRequest) {
  ...
  // duplicate it here (rather than shelling out to `cahi update --check`)
  ...
  if (activeCount > 0) {
    return NextResponse.json<UpdateResponse>(
      {
        ok: false,
        message: `${activeCount} session${activeCount === 1 ? "" : "s"} active. Run \`cahi stop\` first, then click Update again.`,
  ...
  // Spawn `cahi update` detached so this request can return before the install
  // tears down the dashboard process. We rely on PATH resolution because the
  // user installed `cahi` themselves — there's no canonical install location.
  //
  // `shell: isWindows()` is required so PATHEXT gets consulted on Windows —
  // npm's `cahi` shim is `cahi.cmd`, and Node.js does not look at PATHEXT for
  // non-shell spawns, so the bare `cahi` lookup would silently ENOENT on every
  ...
  // case where `cahi` isn't on PATH), NOT as a sync throw — without an explicit
  ...
  try {
    ...
    return NextResponse.json<UpdateResponse>(
      {
        ok: false,
        message: `Failed to spawn 'cahi update': ${err instanceof Error ? err.message : String(err)}`,
      },
      { status: 500 },
    );
  }
  ...
  return NextResponse.json<UpdateResponse>(
    {
      ok: true,
      message:
        "Update started. The dashboard will restart once the new version is installed; re-run `cahi start` to resume.",
    },
    { status: 202 },
  );
}
