...
- 7d9b862: Replace Claude Code terminal-regex activity detection with platform-event hooks (#1941).
  ...
  **`@contaazul/cahi-plugin-agent-claude-code`** now installs two scripts per
  workspace:
  ...
  - `activity-updater` — new; registered on every hook that carries
    ...
    JSONL entry to `{workspace}/.cahi/activity.jsonl` with `source: "hook"`.
  ...
  **`@contaazul/cahi-core`** extends `ActivityLogEntry.source` and
...
- faaddb1: Sessions whose PRs are detected as merged now auto-terminate (tmux kill + worktree remove + metadata archive) instead of lingering in the active `sessions/` directory with a `merged` status. `cahi status` and `cahi session ls` stay clean without an external watchdog.
  ...
  Opt out or tune via the new top-level `lifecycle` config in `cahi.yaml`:
