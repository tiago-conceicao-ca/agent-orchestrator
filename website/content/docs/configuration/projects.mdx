---
title: Projects
description: "Configure one CAHI project: agent, runtime, workspace, setup commands, rules, tracker, SCM, and per-role overrides."
---

A project is one repository that CAHI can run agents against. The global registry remembers the project identity; the local `cahi.yaml` controls how sessions behave inside that repo.
...
`agent` selects the coding agent CAHI launches for worker sessions.
...
`workspace` controls how CAHI isolates each session's code.
...
If a `postCreate` command fails, CAHI does not start the agent for that session. Keep these commands deterministic.
...
If both are set, CAHI includes both.
...
CAHI usually infers tracker and SCM from the registered repository. Override them only when needed.
...
Run `cahi start` from the repository root so CAHI can register the project. If the repo moved, remove and re-add or relink the project instead of editing `storageKey` manually.

**The agent starts without environment variables**  
Add `.env` or the relevant tool config directory to `symlinks`. CAHI does not copy secrets into worktrees by default.
...
Make sure the corresponding CLI or token is authenticated for the plugin you use. CAHI does not store provider tokens in project config.

## Next Steps

<Cards>
  <Card title="Reactions" href="/docs/configuration/reactions" description="Configure what CAHI does when CI, reviews, or agent state changes need attention." />
