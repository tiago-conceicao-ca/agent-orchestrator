---
title: Reactions
description: Configure what CAHI does when CI fails, reviews arrive, agents get stuck, or PRs become ready.
---

Reactions are CAHI's automatic responses to session events. They decide whether CAHI should message an agent, notify a human, or mark a PR as ready for a merge action.

You do not need to configure reactions to start. CAHI ships with defaults for CI failures, review comments, merge conflicts, stuck agents, input prompts, and completed sessions.

## Common Recipes

### Watch Without Auto-Recovery

Use this while onboarding CAHI to a sensitive repo. You still see notifications, but CAHI stops sending recovery prompts to agents automatically.
...
`auto-merge` is currently an intent flag, not a bypass. CAHI does not ignore branch protection, reviews, or failing checks.
</Callout>
...
| Field | Type | Purpose |
|-------|------|---------|
| `auto` | `boolean` | Whether CAHI should perform the automated action |
| `action` | `send-to-agent` \| `notify` \| `auto-merge` | What CAHI does when the reaction fires |
...
1. CAHI sends the configured `ci-failed.message` when the session first enters a failed-CI state.
2. On the next poll, CAHI sends a structured follow-up with the failing check names, statuses, and URLs.
...
CAHI throttles review-comment lookups to avoid hammering GitHub or GitLab. Pending human comments and known automation comments are fetched at most once every two minutes per session.

Force a check when you need one immediately:

```bash
cahi review-check <session-id>
```
...
<Cards>
  ...
  <Card title="Webhook notifier" href="/docs/plugins/notifiers/webhook" description="Send CAHI events to any HTTPS endpoint." />
