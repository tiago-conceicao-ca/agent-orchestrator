---
title: Dashboard
description: Use the CAHI dashboard to monitor agents, respond to sessions, review PRs, manage projects, and open live terminals.
---

The CAHI dashboard is the control room for running agents. It shows what each session is doing, which PRs need review, where agents are blocked, and which projects are active.
...
Open `/prs` to see every PR created by CAHI-managed sessions.
...
Project settings let you edit behavior fields without changing the repository identity. That means you can change things like agent, runtime, tracker, SCM, and reactions, but CAHI keeps the registered path, storage key, and repository identity stable.
...
CAHI has two terminal backends:
...
The dashboard receives live session updates over `/api/events`. CAHI sends snapshots every five seconds. When sessions are added or removed, the dashboard refreshes the full session list from `/api/sessions`.
...
For remote access, prefer Tailscale and keep CAHI off the public internet. See [Remote access](/docs/configuration/remote-access).
...
<Cards>
  <Card title="Quickstart" href="/docs/quickstart" description="Start CAHI, spawn a session, and handle the first PR." />
