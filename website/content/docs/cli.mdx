---
title: CLI reference
description: Every `cahi` command, every flag, drawn from the source.
---
...
├── update                      Check for updates and upgrade CAHI
├── init                        (deprecated — use cahi start)
...
Positional `[project]` — project id, local path, or repo URL. If omitted and no config exists in `cwd`, CAHI creates `cahi.yaml`.

If CAHI is already running, you'll get an interactive prompt to open/restart/spawn-new/quit. Non-TTY callers get info + `process.exit(0)`.
...
| `--all` | — | Stop all running CAHI instances on this machine |
...
<Callout type="warn">
  The old two-arg form `cahi spawn &lt;project&gt; &lt;issue&gt;` is rejected with an error. Use `-p` or run from inside a worktree.
</Callout>

### `cahi batch-spawn <issues...>`
...
If the build looks stale you'll see a suggestion to run `cahi dashboard --rebuild`.
...
Runs `cahi-doctor.sh` for shell-level checks, then TypeScript checks (version freshness, plugin resolution, notifier connectivity). OpenClaw is the only notifier probed without side effects.

### `cahi update`

> Check for updates and upgrade CAHI to the latest version
...
Behavior depends on how CAHI was installed — git, npm-global, pnpm-global, or unknown (manual instructions printed).

### `cahi config-help`

> Show config schema and guide for creating `cahi.yaml`
...
### `cahi session claim-pr <pr> [session]`
...
If `[session]` is omitted, falls back to `CAHI_SESSION_NAME` / `CAHI_SESSION` env vars.

### `cahi session restore <session>`
...
### `cahi session remap <session>`

> Re-discover and persist OpenCode session mapping for an CAHI session
...
Notifier setup commands that write CAHI config support `--routing-preset <preset>`.
Valid presets are `urgent-only`, `urgent-action`, and `all`. In interactive
mode, CAHI asks which notification priorities the notifier should receive before
writing `notificationRouting`.

| Command | Purpose |
|---|---|
| `cahi setup dashboard` | Configure dashboard notification retention and routing |
| `cahi setup desktop` | Configure native desktop notifications |
| `cahi setup webhook` | Configure a generic HTTP webhook |
| `cahi setup slack` | Configure a Slack incoming webhook |
| `cahi setup discord` | Configure a Discord incoming webhook |
| `cahi setup composio` | Open the interactive Composio setup hub |
| `cahi setup composio-slack` | Configure Slack through Composio |
| `cahi setup composio-discord` | Configure Discord webhook mode through Composio |
| `cahi setup composio-discord-bot` | Configure Discord bot mode through Composio |
| `cahi setup composio-mail` | Configure Gmail through Composio |
| `cahi setup openclaw` | Configure OpenClaw gateway notifications |

### `cahi setup openclaw`

> Connect CAHI notifications to an OpenClaw gateway
...
| `--refresh` | — | Reuse existing values and rewrite CAHI config |
...
Interactive mode (TTY) uses `@clack/prompts` with review/change steps for the gateway URL, OpenClaw config path, and routing preset. OpenClaw owns the token in `hooks.token`; CAHI does not generate it or write shell-profile exports.
...
### `cahi plugin search <query>`

> Search the bundled marketplace catalog

No flags.

### `cahi plugin create [directory]`

> Scaffold a new CAHI plugin package
...
### `cahi plugin uninstall <reference>`
...
| Variable | Purpose |
|---|---|
| `CAHI_DATA_DIR` | Override the `~/.cahi` base directory. |
| `CAHI_LOG_LEVEL` | `error` · `warn` · `info` · `debug` · `trace` |
| `CAHI_CONFIG_PATH` | Absolute path to a specific `cahi.yaml`. Overrides CWD-based search. |
| `CAHI_SESSION_ID` · `CAHI_SESSION_NAME` | Auto-set inside spawned sessions; used by `claim-pr` default |
| `CAHI_PROJECT_ID` | Set by CAHI inside spawned sessions. Used by `cahi spawn` to resolve the active project. |
