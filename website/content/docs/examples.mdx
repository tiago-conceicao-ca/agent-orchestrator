...
All examples live in the [`examples/` directory](https://github.com/contaazul/cahi/tree/main/examples) of the CAHI repository. Pick the one closest to your setup, copy it to your project root as `cahi.yaml`, fill in your repo path and any API keys, and you're ready to spawn agents.
...
- No `defaults:` block needed — CAHI's built-in defaults (`tmux` runtime, `claude-code` agent, `worktree` workspace) apply automatically.
- Replace `owner/my-app` and `~/my-app` with your actual GitHub slug and local checkout path.

[View raw file on GitHub](https://github.com/contaazul/cahi/blob/main/examples/simple-github.yaml)
...
[View raw file on GitHub](https://github.com/contaazul/cahi/blob/main/examples/linear-team.yaml)
...
[View raw file on GitHub](https://github.com/contaazul/cahi/blob/main/examples/multi-project.yaml)
...
- `ci-failed.retries: 3` lets CAHI send the agent back to fix CI up to three times before escalating.
...
[View raw file on GitHub](https://github.com/contaazul/cahi/blob/main/examples/auto-merge.yaml)
...
[View raw file on GitHub](https://github.com/contaazul/cahi/blob/main/examples/codex-integration.yaml)
...
<Cards>
  <Card title="Configuration Reference" href="/docs/configuration" description="Full reference for every field in cahi.yaml." />
