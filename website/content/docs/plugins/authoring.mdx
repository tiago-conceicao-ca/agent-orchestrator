...
description: Write a custom runtime, agent, workspace, tracker, SCM, notifier, or terminal plugin for CAHI.
---
...
Return `true` when the system has what your plugin needs — a binary on PATH, an environment variable, a running process. CAHI uses this to warn about missing dependencies at startup rather than at first use.

### Default export

```typescript
import type { PluginModule, Notifier } from "@contaazul/cahi-core";
...
- **`name`** must match the suffix of your package name. `@acme/cahi-plugin-notifier-pagerduty` → `name: "pagerduty"`. The registry looks up plugins by `slot:name` key.
...
## Create a plugin with `cahi plugin create`

The fastest way to start is `cahi plugin create`. It runs an interactive prompt (using `@clack/prompts`) and writes a ready-to-build package.
...
```bash
cahi plugin create ./my-plugins/pagerduty \
  --slot notifier \
  --name "PagerDuty" \
  --description "Route urgent alerts to PagerDuty" \
  --author "Alice" \
  --package-name "cahi-plugin-notifier-pagerduty"
```
...
| Author | `Alice` |
| Package name | `cahi-plugin-notifier-pagerduty` (pre-filled from slot + name) |
...
├── README.md          # CAHI config snippets for local and npm installs
...
Replace the placeholder `create()` body with a real implementation. Import the slot interface from `@contaazul/cahi-core`:

```typescript
import type { PluginModule, Notifier } from "@contaazul/cahi-core";
import { validateUrl } from "@contaazul/cahi-core";
```
...
plugins:
  - name: pagerduty
    source: registry
    package: "cahi-plugin-notifier-pagerduty"
    version: "^1.0.0"
```
...
All interfaces are defined in `packages/core/src/types.ts` and exported from `@contaazul/cahi-core`.
...
| `getEnvironment` | Env vars for the process (must include `~/.cahi/bin` in PATH) | No |
...
CAHI resolves plugins through four mechanisms, tried in this order:

### 1. Bundled

Plugins under `packages/plugins/` in the CAHI monorepo are registered automatically. No config is required.

### 2. Registry / npm (`source: registry` or `source: npm`)

An npm package. CAHI installs it into its internal plugin cache on first use and keeps the version pinned in `cahi.yaml`.
...
Use `cahi plugin install cahi-plugin-notifier-pagerduty` to add this block automatically.

### 3. Local path (`source: local`)

A filesystem path. Useful during development. Path is resolved relative to `cahi.yaml`.
...
CAHI will import `dist/index.js` (falling back to `index.js`) from that directory.
...
## `cahi plugin` subcommands

| Command | Description |
|---------|-------------|
| `cahi plugin list` | List plugins from the bundled marketplace catalog. Add `--installed` to show only what's in your config. Filter by slot with `--type <slot>`. Add `--refresh` to pull the latest catalog from the registry. |
| `cahi plugin search <query>` | Search the bundled catalog by name, package, description, or slot. |
| `cahi plugin create [dir]` | Scaffold a new plugin package interactively. |
| `cahi plugin install <reference>` | Install a plugin by marketplace ID, package name, or local path. Writes the entry to `cahi.yaml`. |
| `cahi plugin update [reference]` | Update an installer-managed plugin. Pass `--all` to update everything. |
| `cahi plugin uninstall <reference>` | Remove a plugin from the config. |

---

## Core utilities available to plugins

All utilities are exported from `@contaazul/cahi-core`. The source lives in `packages/core/src/index.ts`.
...
| `readLastActivityEntry` | Read the last entry from the CAHI activity JSONL (`{workspace}/.cahi/activity.jsonl`). |
...
| Export | Purpose |
|--------|---------|
| `setupPathWrapperWorkspace` | Install `~/.cahi/bin/gh` and `~/.cahi/bin/git` PATH wrappers and write `.cahi/AGENTS.md` in the workspace. Required for PATH-wrapper agents (Codex, Aider, OpenCode). |
| `buildAgentPath` | Prepend `~/.cahi/bin` to a PATH string. |
...
For agents that don't have a native session API (most new agents), skip step 3 and rely entirely on the CAHI activity JSONL written by `recordActivity`. The full specification — including hook patterns, `isProcessRunning` requirements, and the 7 required test cases — is in the "Agent Plugin Implementation Standards" section of `CLAUDE.md` in the project root.
...
<Cards>
  ...
  <Card title="Configuration reference" description="How plugins are referenced in cahi.yaml." href="/docs/configuration" />
</Cards>
