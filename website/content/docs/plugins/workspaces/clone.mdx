...
workspace: clone
workspaceConfig:
  cloneDir: ~/.cahi-clones       # default
```

| Config key | Default | What it does |
|---|---|---|
| `cloneDir` | `~/.cahi-clones` | Base directory for all clones |

## How it works

- On spawn, CAHI runs `git clone --reference <source> <cloneDir>/{sessionId}`. The `--reference` flag means the new clone borrows objects from your existing checkout — much faster than a cold clone.
