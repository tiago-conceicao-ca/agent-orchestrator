...
[OpenClaw](https://github.com/composio-dev/openclaw) is a local gateway that routes CAHI events into whatever personal alert channel you've wired up — phone push, email, a local sound, etc. If it's running on your machine, this notifier uses it.

## Setup

Run CAHI's interactive helper:
...
4. Write only `cahi.yaml`; CAHI does not generate the token or write shell-profile exports.
5. Let you review and change the URL, OpenClaw config path, or routing preset before saving.

Non-interactive variant:

```bash
cahi setup openclaw --non-interactive --url http://127.0.0.1:18789/hooks/agent --routing-preset urgent-action
```
...
| `token` | — | Remote/manual fallback. Avoid for local setups because it stores the secret in CAHI config |
...
| `healthSummaryPath` | — | Optional local path where OpenClaw writes a summary CAHI can pick up |

## Token precedence

Token is read in this order:

1. `token` in `cahi.yaml` if explicitly configured
2. `hooks.token` from `openclawConfigPath`
3. Env var `OPENCLAW_HOOKS_TOKEN` as a legacy fallback

`cahi setup openclaw --status` probes the OpenClaw gateway and token. To send a
real CAHI test notification through the configured notifier, run:

```bash
cahi notify test --to openclaw --template basic
```
