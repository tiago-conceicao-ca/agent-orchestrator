...
CAHI asks which desktop backend to use and which notification priorities desktop
...
|---|---|---|
| `backend` | `auto` | `cahi-app`, `terminal-notifier`, `osascript`, or auto fallback |
| `dashboardUrl` | dashboard port | URL opened from desktop notification actions |
| `appPath` | macOS app install path | Custom CAHI Notifier.app path |
