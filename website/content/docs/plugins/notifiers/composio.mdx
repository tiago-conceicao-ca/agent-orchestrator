...
The hub lets you choose Slack, Discord webhook, Discord bot, or Gmail. CAHI asks for the Composio API key, `userId`, required target details, which notification priorities the notifier should receive, then reviews the config before writing. When you run `cahi setup composio`, the selected app is written to the canonical `notifiers.composio` target. You can skip the app picker with `--slack`, `--discord-webhook`, `--discord-bot`, or `--gmail`. Discord webhook mode does not need a Composio connected account; Discord bot mode uses a bot token once to create a connected account and stores only the resulting `connectedAccountId`. Gmail mode uses an existing Gmail connected account, or creates a Composio connect link from an existing Gmail auth config.

Dedicated commands use the same guided setup but write app-specific targets:

```bash
cahi setup composio-slack        # writes notifiers.composio-slack
cahi setup composio-discord      # writes notifiers.composio-discord
cahi setup composio-discord-bot  # writes notifiers.composio-discord-bot
cahi setup composio-mail         # writes notifiers.composio-mail
```
...
cahi setup composio-slack --user-id aoagent --channel "#agents" --non-interactive
```
...
```bash
cahi setup composio-slack --connected-account-id ca_... --non-interactive
```
...
```bash
export COMPOSIO_API_KEY=ak_...
cahi setup composio-discord --webhook-url "https://discord.com/api/webhooks/..." --non-interactive
```
...
```bash
export COMPOSIO_API_KEY=ak_...
cahi setup composio-discord-bot --channel-id "1234567890" --bot-token "$DISCORD_BOT_TOKEN" --non-interactive
```

The bot token is used once to create a Composio connected account. CAHI writes only the resulting `connectedAccountId`.

Run the Gmail setup when you want CAHI notifications by email:

```bash
export COMPOSIO_API_KEY=ak_...
cahi setup composio-mail --email-to alerts@example.com --non-interactive
```

Connect Gmail in Composio first, or choose Gmail in `cahi setup composio` and generate a connect link from an existing Gmail auth config. CAHI does not create Gmail OAuth/auth configs. The setup command finds an active Gmail connected account for the configured `userId` and writes its `connectedAccountId`. If multiple Gmail accounts are available, pass the one CAHI should use:

```bash
cahi setup composio-mail --email-to alerts@example.com --connected-account-id ca_...
```

You can also ask CAHI to print a Composio Gmail connect URL:

```bash
cahi setup composio-mail --email-to alerts@example.com --connect
```

CAHI uses an existing Composio Gmail auth config for that link. If you need a specific custom Gmail auth config, pass it explicitly:

```bash
cahi setup composio-mail --email-to alerts@example.com --connect --auth-config-id ac_...
```

If Google blocks a Gmail connection, fix the Gmail auth config in Composio, then rerun `cahi setup composio-mail`.
...
If `mode` is omitted for Discord, CAHI uses `webhook` when `webhookUrl` is present and `bot` otherwise.
