...
5. Copy the webhook URL and paste it into CAHI.

The command sends a test message before writing CAHI config.
Before saving, CAHI also asks which notification priorities Discord should receive:
...
```bash
cahi setup discord --status
cahi setup discord --refresh
```
...
| `username`     | optional | `CAHI` | Override webhook default username per message                       |
...
CAHI uses Discord embeds, not plain text. Each event shows:
...
```bash
cahi notify test --to discord --template basic
cahi notify test --to discord --template merge-ready --actions
```
