...
Notifiers deliver CAHI events — session stuck, PR opened, review requested, CI failed — to wherever you actually pay attention.
...
urgent-only`, `urgent-action`, or `all`. CAHI does not dedupe across channels, so
route only the priorities each destination should receive.

## Testing

```bash
cahi notify test --to slack --template basic
cahi notify test --route urgent
cahi notify test --all --dry-run
```
...
| Event | When |
|---|---|
| `session.spawned` | `cahi spawn` succeeds |
