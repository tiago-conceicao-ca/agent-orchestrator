...
Point it at any HTTPS endpoint. CAHI POSTs a JSON event. Use this for PagerDuty-style integrations, custom dashboards, or anything we don't have a dedicated notifier for.
...
paste a different endpoint, go back, or cancel before CAHI sends the setup test.
Before saving, CAHI also asks which notification priorities the webhook should
...
Free-form messages sent via `cahi send` arrive as:

```json
{
  "type": "message",
  "message": "Hello from CAHI",
  "context": {}
}
```
...
- Restrict inbound traffic to your CAHI host via IP allowlist or VPN.
...
- You're building an internal CAHI dashboard of your own.
