...
5. Pick the channel CAHI should post to, authorize, and copy the webhook URL.

The command sends a test message before writing CAHI config.
Before saving, CAHI also asks which notification priorities Slack should receive:
...
```bash
cahi setup slack --status
cahi setup slack --refresh
```
...
| `username`   | optional | `CAHI` | Requested display name                                                                              |
