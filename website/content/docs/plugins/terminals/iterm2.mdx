...
The macOS attach experience. When you run `cahi open`, it opens each session in a new iTerm2 tab (or window, with `--new-window`).
...
- **"open-iterm-tab: command not found".** Reinstall CAHI (`npm install -g @contaazul/cahi`) — the helper binary lives in the cahi package.
