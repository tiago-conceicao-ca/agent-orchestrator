...
The **terminal** plugin is what `cahi open` and `cahi session attach` use. If you only ever look at the dashboard, you don't need to think about it.
...
CAHI picks `iterm2` on macOS when iTerm2 is installed; falls back to `web` otherwise. You can pin the choice:
