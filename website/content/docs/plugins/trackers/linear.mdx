...
description: Linear issues. Direct API key or Composio-mediated — CAHI picks the right transport automatically.
---
...
CAHI detects this and routes Linear calls through Composio — no separate Linear token needed. `COMPOSIO_ENTITY_ID` is optional; if omitted it defaults to `"default"`.
...
When CAHI spawns an agent for a Linear issue it:
...
You can customise the format globally via Linear's branch format settings — CAHI will pick up whatever Linear generates automatically.

## Issue identifiers

Linear uses `TEAM-42`-style identifiers, not numbers. `cahi spawn TEAM-42` works as you'd expect.
