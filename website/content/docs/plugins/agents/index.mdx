...
description: Pick the agent that writes the code. CAHI supports five today, and they're swappable per-project.
---

The agent is the piece that actually writes code. Everything else in CAHI — worktrees, runtimes, notifiers — is plumbing around it.

<Callout type="info">
**PR metadata is captured automatically.** Non-Claude-Code agents (Codex, Cursor, Aider, OpenCode) use `~/.cahi/bin/gh` and `~/.cahi/bin/git` PATH wrappers that intercept commands like `gh pr create` and record the resulting PR number and branch so the dashboard stays in sync. Claude Code uses PostToolUse hooks in `.claude/settings.json` instead — the same metadata is written through Claude Code's native hook system. Both approaches are set up transparently by CAHI; you don't need to configure anything.
</Callout>
...
## How CAHI talks to them

Two patterns exist:

- **Agent-native hooks** — Claude Code ships a PostToolUse hook in `.claude/settings.json`. CAHI's workspace bootstrap installs it; the agent writes activity events directly to CAHI's JSONL.
- **PATH wrappers** — Codex, Cursor, Aider, and OpenCode don't have a hook system. CAHI installs small shell wrappers in `~/.cahi/bin/gh` and `~/.cahi/bin/git` and prepends them to `PATH`. When the agent runs `gh pr create` or `git commit`, the wrapper records metadata (PR number, branch, etc.) so the dashboard knows what the agent did.

Neither approach requires you to change your agent's config — CAHI sets it up transparently.
