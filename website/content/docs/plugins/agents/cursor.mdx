...
The [Cursor Agent CLI](https://cursor.com/docs) brings Cursor's editing model to a terminal. CAHI spawns it per session, wraps `gh`/`git`, and tracks activity via the CAHI activity log.
...
- **Launch:** CAHI runs `agent` inside the worktree.
- **Activity tracking:** CAHI writes a JSONL to `{workspace}/.cahi/activity.jsonl` each poll cycle based on terminal output classification. The dashboard reads from there.
- **PR + git tracking:** PATH wrappers at `~/.cahi/bin/gh` / `~/.cahi/bin/git` intercept calls to record PR metadata.
...
<Accordions>
  <Accordion title="CAHI says Cursor Agent isn't detected">
  ...
  <Accordion title="Dashboard never leaves `active`">
    Cursor doesn't emit its own session JSONL. CAHI classifies based on terminal output — if your terminal is being captured by another tool, the classification can drift. `cahi doctor` will flag this.
  </Accordion>
</Accordions>
