...
- CAHI spawns the agent via Node's `child_process.spawn` with `shell: true`.
...
- **No tmux attach.** `cahi session attach` doesn't work. Use the dashboard terminal instead.
- **No reconnecting to the agent's TTY.** If you `cahi stop` the orchestrator, the child process goes with it. The agent's own session-resume features (Claude `--resume`, Codex `resume`, etc.) still work — that's a different layer.
