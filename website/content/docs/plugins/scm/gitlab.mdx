...
CAHI normalises these internally — the dashboard and lifecycle state machine don't know which you use.
...
- **URL**: `https://<your-cahi-host>/api/webhooks`
...
**Verification**: GitLab sends the configured secret as a literal string in `X-Gitlab-Token`. CAHI compares this value directly (no HMAC — unlike GitHub's SHA-256 approach).

Polling is still active as a fallback — webhooks are a latency optimisation, not a hard dependency.

## Automated review authors

CAHI ignores review comments from known bot accounts so they don't block the merge-readiness check. The full list:
...
See [Review loop — bot detection](/docs/guides/review-loop#automated-review-bugbot-detection) for how CAHI uses this list during the review-pending → mergeable transition.
