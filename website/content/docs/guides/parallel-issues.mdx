...
The whole point of CAHI is running agents in parallel. Each session gets its own git worktree, so agents can't stomp each other's branches.

## Batch spawn

`cahi batch-spawn` takes one or more issue identifiers:

```bash
cahi batch-spawn 42 43 44
```
...
For a quick interactive version, just run `cahi spawn` a few times — `batch-spawn` adds the duplicate detection.
...
```bash
cahi spawn 42 --agent claude-code
cahi spawn 42 --agent codex
```

CAHI creates two distinct sessions. Compare the PRs side by side.

## Isolation guarantees

| What's isolated | How |
|---|---|
| File edits | Each session has its own worktree under `~/.worktrees/{projectId}/{sessionId}/`. Session metadata lives under `~/.cahi/{hash}-{projectId}/sessions/{sessionId}` — the worktree itself is in `~/.worktrees/`. |
| Branch name | CAHI names the branch after the session ID, avoiding collisions |
...
cahi status -p my-repo --json
```

## When you want fewer

- `cahi session kill <sessionId>` — kill one
- `cahi session cleanup` — kill everything whose PR merged or issue closed (safe; archives metadata)
- `cahi session cleanup --dry-run` — preview first

<Callout type="info">
  Need them all gone? `cahi stop --all` halts every running CAHI instance on this machine.
</Callout>
