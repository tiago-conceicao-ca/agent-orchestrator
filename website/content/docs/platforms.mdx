---
title: Platforms
description: Choose the right CAHI runtime, terminal, and notification setup for macOS, Linux, Windows, and remote hosts.
---

import { Callout } from "fumadocs-ui/components/callout";

CAHI's core workflow is the same on every platform: create an isolated workspace, run an agent, track the PR, react to CI and review events, and show progress in the dashboard.
...
| Container or remote VM | `process` or `tmux` | Slack, Discord, webhook | Use persistent storage for CAHI data and protect the dashboard with your own auth. |
...
| macOS idle sleep prevention while CAHI runs | Supported |

<Callout type="info" title="Lid-close sleep still wins">
  CAHI can prevent idle sleep on macOS while agents run, but it cannot override hardware lid-close sleep. Use normal clamshell mode if you need the machine available while closed.
</Callout>
...
If you are running CAHI on a headless Linux host, prefer Slack, Discord, or webhook notifications over desktop notifications.
...
| Capability | Status |
| --- | --- |
| `cahi start`, `cahi stop`, `cahi dashboard` | Supported |
| `cahi spawn` and `cahi spawn --prompt` | Supported through the process runtime |
...
| Slack, Discord, webhook, and Composio notifiers | Supported |
...
| tmux attach commands | Not available | Dashboard terminal and CAHI session commands |

Use PowerShell or Git Bash for normal CLI commands. If a command behaves differently because of shell quoting, put longer instructions in a file and send them with `cahi send --file`.

## Containers And Remote Hosts

CAHI can run on a remote machine or inside a container, but the dashboard has no built-in authentication. Put it behind your own access layer before exposing it beyond localhost.
...
- Mount or preserve CAHI's data directory so session metadata survives restarts.
...
Start with the recommended runtime for your OS. Change it only when the default runtime does not match where CAHI is running.
